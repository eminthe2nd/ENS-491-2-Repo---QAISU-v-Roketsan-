...
class QualityDecisionSystem:
    ...
    def prepare_for_modeling(self, test_size=0.2):
        ...
        num_cols = [c for c in [
            'INSPECTION_RESULT', 'LOWER_LIMIT', 'UPPER_LIMIT', 'LIMIT_INTERVAL',
        ...
        self.num_cols_ = num_cols  # <<< KRİTİK: train’de fit edilen numeric kolonları sakla

        if num_cols and not self.X_train.empty:
            try:
                # numeric garanti
                for c in num_cols:
                    self.X_train[c] = pd.to_numeric(self.X_train[c], errors='coerce')
                    self.X_test[c]  = pd.to_numeric(self.X_test[c],  errors='coerce')

                self.X_train.loc[:, num_cols] = self.scaler.fit_transform(self.X_train[num_cols].fillna(0))
                self.X_test.loc[:, num_cols]  = self.scaler.transform(self.X_test[num_cols].fillna(0))
            except Exception as e:
                logger.warning(f"Scaling failed; continuing without scaling numeric columns. Reason: {e}")
                self.scaler = None
                self.num_cols_ = []
    ...
    def process_all_data(self):
        ...
        if self.scaler is not None and hasattr(self.scaler, "mean_") and self.num_cols_:
            try:
                # full df’de de numeric garanti + aynı kolon seti + aynı sıra
                for c in self.num_cols_:
                    if c in df.columns:
                        df[c] = pd.to_numeric(df[c], errors='coerce')

                df.loc[:, self.num_cols_] = self.scaler.transform(df[self.num_cols_].fillna(0))
            except Exception as e:
                logger.warning(f"Full-scaling failed; continuing without. Reason: {e}")
